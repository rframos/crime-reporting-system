...
class Incident(db.Model):
    ...
    confidence = db.Column(db.Integer, default=0) # Added back for reports.html
...
def background_train():
    with app.app_context():
        try:
            ...
            train_gen = datagen.flow_from_directory(app.config['TRAIN_FOLDER'], target_size=(32, 32), batch_size=8, class_mode='categorical')
...
def report_incident():
    ...
    if file:
        ...
        new_inc = Incident(incident_type=ptype, latitude=float(lat or 0), longitude=float(lng or 0), image_url=filename, confidence=conf)
        db.session.add(new_inc); db.session.commit()
        flash(f"Report Submitted: {ptype}", "success")
...
def cnn_admin():
    ...
    dataset = {cat.name: os.listdir(os.path.join(app.config['TRAIN_FOLDER'], cat.name)) for cat in categories if os.path.exists(os.path.join(app.config['TRAIN_FOLDER'], cat.name))}
...
def upload_training_images():
    ...
    if files and cat:
        ...
        for f in files:
            if f.filename: f.save(os.path.join(path, secure_filename(f.filename)))
    return redirect(url_for('cnn_admin'))
